...
# mealy machine with non-determinism in the input abab -> 0000 and 0001
mealy_non_det = Mealy(
    ['A', 'B', 'C', 'D'],
    ['a', 'b'],
    ['0', '1'],
...
# mealy machine deterministic -> works on learnlib :)
mealy = Mealy(['A', 'B','D'],
                ['a', 'b'],
                ['0', '1'],
                {
...
#print(mealy)
#print(mealy.get_output_from_string(string))
...
with open('traces.txt', 'w') as f:
    for i in range(20): 
        ...
        s = random.choice([p for p in itertools.product(x, repeat=num)])
        string = convert_fromCh_toSt(s)
        print(string)
        print(mealy.get_output_from_string(string))
        
        lines = [string + ' - > ' + mealy.get_output_from_string(string)]
        for line in lines:
            f.write(line)
            f.write('\n')
