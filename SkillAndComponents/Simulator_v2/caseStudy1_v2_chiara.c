...
void *visitExecution(void *threadid)
{
	...
	while (1)
	{
		...
		//fflush(stdout);
		// execution stops on success
		assert(!(_btRes == STATUS_SUCCESS));
		// execution stops if the battery level reaches zero percent
		assert(!(simInfo.batteryLevel <= 0));
#endif
...
//FILE *choosen_parameter_file = fopen("choosen_param.txt", "a");
#if defined(SIMULATE) || defined(SIMULATE_CHIARA)|| defined(MONITOR) || defined(MONITOR2) || defined(MONITOR3)
	// the robot will trigger to go to charging station when the battery level is 500
	// battery drop is between 2 and 8
	// if the level is 2, 3 or 4 the robot will reach the kitchen without to go to charging station
	srandom(time(NULL));
	simInfo.isArrived = false;
	simInfo.atDestination = 0;
	simInfo.atChargingStation = 0;
	...
	simInfo.cyclesToDestination = simInfo.cyclesToDestinationPlanned;
	simInfo.batteryDrop = randomInteger(2, 8);  
	simInfo.batteryRise = 100;
	// The robot charging routine starts if batteryLevel is below 500 (early warning)
	// If batteryDrop is 10 and cyclesToChargingPlanned is greater than 61,
	// the robot will not be able to reach the charging station
	// simInfo.cyclesToChargingPlanned = 32;
	simInfo.cyclesToChargingPlanned = randomInteger(1, 10);  
	simInfo.cyclesToCharging = simInfo.cyclesToChargingPlanned;
